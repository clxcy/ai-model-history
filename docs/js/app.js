...
function createTimelineCard(model, isLeft, index) {
    ...
    // Construct Date Label HTML
    const dateLabelHtml = `<div class="timeline-date-label timeline-date-only">${formattedDate}</div>`;

    // Determine layout based on side
    let leftContent = '';
    let rightContent = '';

    if (isLeft) {
        leftContent = cardHtml;
        rightContent = dateLabelHtml;
    } else {
        leftContent = dateLabelHtml;
        rightContent = cardHtml;
    }

    return `
        <div class="timeline-row" data-aos="fade-up" data-aos-delay="${index * 50}">
